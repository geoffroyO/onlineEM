...
def brentq(f):
    ...
    def x(a, b, args=(), xtol=2e-14, maxiter=200):
        ...
        def _f6(x):
            ...
            x['stry'] = -x['fcur'] * (x['fblk'] * dblk -
                                      x['fpre'] * x['dpre']) / _tmp
            return x

        def _f3(x):
            x = jax.lax.cond(x['xpre'] == x['xblk'], _f5, _f6, x)
            k = jnp.min(jnp.array([abs(x['spre']),
                                   3 * abs(x['sbis']) - x['delta']]))
            j = 2 * abs(x['stry']) < k
        ...
        def body_fun(x):
            ...
            x = jax.lax.cond(abs(x['fblk']) < abs(x['fcur']),
                             _f2, lambda a: a, x)
            ...
            x = jax.lax.cond(jnp.logical_and(abs(x['spre']) > x['delta'],
                                             abs(x['fcur']) < abs(x['fpre'])),
                             _f3, _f4, x)
            x['xpre'] = x['xcur']
            x['fpre'] = x['fcur']
            x['xcur'] += jnp.where(abs(x['scur']) > x['delta'],
                                   x['scur'], jnp.where(x['sbis'] > 0,
                                                        x['delta'],
                                                        -x['delta']))
        ...
        def cond_fun(R):
            return jnp.logical_and(R['status'] != ECONVERGED,
                                   R['itr'] <= maxiter)

        R = dict(root=root, status=status, xpre=xpre,
                 xcur=xcur, fpre=fpre, fcur=fcur,
